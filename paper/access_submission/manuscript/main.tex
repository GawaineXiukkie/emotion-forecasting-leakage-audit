...
\address[1]{School of Computer Sciences, Universiti Sains Malaysia, 11800 Penang,
Malaysia (e-mail: wenbin@student.usm.my)}
\markboth{Wen: Information-Matched Auditing of Conversational Emotion-Shift Forecasting}
{Wen: Information-Matched Auditing of Conversational Emotion-Shift Forecasting}
\corresp{Corresponding author: Bin Wen (e-mail: wenbin@student.usm.my).}
...
$[+0.199,+0.371]$) or MELD ($+0.208$, $[+0.151,+0.265]$); both two-sided bootstrap
$p<.001$. EmoryNLP shows no supported difference ($-0.025$, $[-0.122,+0.066]$, $p=.579$).
...
appearance of every speaker, leaving 81.7--98.1\% as many test decisions as the immediate target,
...
($+0.160$ to $+0.167$ ROC--AUC; all adjusted $p=.0144$) and smaller elsewhere. DialogueRNN on
MELD is the only other comparison that survives the 72-test familywise correction
($\Delta=+0.041$, adjusted $p=.0352$). Positive but uncorrected differences on IEMOCAP and
EmoryNLP are evidence of possible benefit, not conclusive wins.
...
All six DailyDialog gains and DialogueRNN's MELD gain survive correction.}
\label{tab:main}
\begin{tabular}{lccccccc}\toprule
...
gains survive joint Holm correction; on MELD, four of the six negative differences survive.}
\label{tab:oracle}
\begin{tabular}{lccccccc}\toprule
...
\textbf{Same-architecture dose response (Fig.~\ref{fig:dose}).} A two-layer Transformer holds
parameters, positional encoding, pooling, optimizer, and checkpoint rule fixed; only its attention
mask changes so that position $n$ sees $k\in\{0,1,2,4,\mathrm{all}\}$ future utterances. The response
is not monotone. MELD rises from .630 at $k=0$ to .721 at $k=1$, then falls to
.675/.655/.640. Duplicate-free DailyDialog similarly rises from .741 to .809 and falls to
.791/.774/.771. The other four configurations vary by at most .033 without a consistent direction.
Thus, exposure to the target utterance can materially inflate performance, but additional future
context need not add signal and may dilute or complicate optimization. Because architecture is held
fixed, this conclusion no longer relies on comparing a look-ahead GRU with a bidirectional GRU.
...
The author used OpenAI Codex to assist with English-language editing, \LaTeX{} formatting, code
review, and the preparation of supplementary analyses. The author reviewed and verified the
manuscript, code changes, experimental results, and conclusions and assumes full responsibility
...
\begin{IEEEbiographynophoto}{Bin Wen}
is with the School of Computer Sciences, Universiti Sains Malaysia, Penang, Malaysia. His current
research focuses on conversational affective computing, multimodal machine learning, and reliable
evaluation protocols for predictive systems.
