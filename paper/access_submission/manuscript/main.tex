...
\textbf{Stronger predicted-label baseline.} The deployable baseline commits to the classifier's
single most likely current label and discards its uncertainty. To test whether the headline gains
merely exploit that discarded uncertainty, a posterior-marginalized control computes
$P(\mathrm{shift}\mid x_n)=\sum_k P(y_n{=}k\mid x_n)\,P(y_{n+1}{\neq}k\mid y_n{=}k,\text{speaker})$,
with the class posterior from a train-only multinomial logistic regression (same feature rows,
scaler, and row cap as the hard baseline's classifier) and the continuation terms from the same
train-fold transition matrix; learned-model scores are the identical cached runs behind
Table~\ref{tab:main}. Marginalization is generally, though not uniformly, stronger: duplicate-free
DailyDialog rises from .580 to .662 and MELD-mm from .584 to .605, while IEMOCAP dips slightly
(.576 to .569). Every DailyDialog and MELD-mm model gain nevertheless remains
corrected-significant in a separately declared 36-comparison Holm family ($+.078$ to $+.085$ and
$+.111$ to $+.118$, respectively). MELD's smaller gains, including DialogueRNN's, do not survive
against this stronger baseline, and EmoryNLP's vanish ($\Delta\leq+.030$, none significant). The
control therefore sharpens rather than overturns the conditional conclusion: the model advantages
that survive information matching also survive posterior marginalization exactly where they were
large to begin with.
...
\item DailyDialog is the most stable positive case: every tested model beats the hard predicted-label,
posterior-marginalized, and gold-label baselines after duplicate removal, across every seed, with
corrected-significant hierarchical inference.
...
\textbf{Information matching is imperfect in the oracle direction.} In the oracle diagnostic, the
gold one-hot label is concatenated to the per-turn feature vector at every position $t \le n$, so a
...
transition baseline's competitiveness under a strict single-label match. The symmetric concern on
the deployable side -- that a hard current-label prediction discards classifier uncertainty a
stronger baseline could exploit -- is addressed directly by the posterior-marginalized control in
\S\ref{sec:exp}, which strengthens the baseline substantially on several corpora without
overturning the corrected-significant DailyDialog and MELD-mm gains. The oracle-side asymmetry
remains a reporting caveat rather than being resolved with a new model family in this submission.
...
Marginalized baseline & \texttt{access\_soft\_baseline} & 36 corrected tests \\
...
review, and the documentation and presentation of supplementary analyses. All experimental
designs, executions, interpretations, and conclusions were independently reviewed and verified
by the authors, who assume full responsibility for the submitted work.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_photos/bin_wen.png}}]{Bin Wen}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_photos/dai_qiao_zhang.png}}]{Dai-Qiao Zhang}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_photos/tien_ping_tan.png}}]{Tien-Ping Tan}
